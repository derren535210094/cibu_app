...
final formattedMoney = formatter.format(int.parse(ap.userModel.money)).replaceAll(',', '');
    setState(() {
...
void _onButtonTap() {
  setState(() {
    ...
    final String formattedTime = DateFormat('kk:mm:ss').format(now); // 24-hour format
    final int decreaseNumber = int.tryParse(transfervalue.text) ?? 0;
    final formatters = NumberFormat("#,###", "en_US");
    final formattedMoneys = formatters.format(int.parse(decreaseNumber.toString())).replaceAll(',', '.');
    ...
    body: SingleChildScrollView(
        child: Container( 
          ...
          // color:  Colors.yellow,
                ...
                child: GradientText('Selamat datang,',
                  style: const TextStyle(
                    fontSize: 25,
                  ),
                  gradientType: GradientType.linear,
                ...
                child: GradientText(ap.userModel.name, // 'Selamat datang,\n\$namauser'
                  style: const TextStyle(
                    fontSize: 25,
                  ),
                  gradientType: GradientType.linear,
    ...
    return LinearGradient(
      ...
      child: ElevatedButton(
                  style: ElevatedButton.styleFrom(
                    ...
                    backgroundColor: !showhome&&!showhistory&&!showtransfer ? const Color.fromRGBO(152,156,236,1)
                    : showhome&&!showhistory&&!showtransfer ? const Color.fromRGBO(152,156,236,1)
                  ...
                  foregroundColor: const Color.fromRGBO(137,146,231,1),
                  elevation: 0,
                  
                  ).merge(
                  ButtonStyle(
        side: !showhome&&!showhistory&&!showtransfer ? MaterialStateProperty.all(const BorderSide(color: Colors.white, width: 2))
        : showhome&&!showhistory&&!showtransfer ? MaterialStateProperty.all(const BorderSide(color: Colors.white, width: 2))
        :null,
      ),
                  ),
...
Container(
      ...
      child: ElevatedButton(
                  style: ElevatedButton.styleFrom(
                    animationDuration: const Duration(milliseconds: 0),
                    backgroundColor: _isTapped2 ? const Color.fromRGBO(152,156,236,1) : const Color.fromRGBO(137,146,231,1),
                    shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(50),
                  ...
                  elevation: 0,
                    ...
                    ButtonStyle(
        side: _ishighlight2 ? MaterialStateProperty.all(const BorderSide(color: Colors.white, width: 2)) : null,
      ),
                  ),
...
Container(
      ...
      child: ElevatedButton(
                  style: ElevatedButton.styleFrom(
                    animationDuration: const Duration(milliseconds: 0),
                    backgroundColor: _isTapped3 ? const Color.fromRGBO(152,156,236,1) : const Color.fromRGBO(137,146,231,1),
                    shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(50),
                  ...
                  elevation: 0,
                    ...
                    ButtonStyle(
        side: _ishighlight3 ? MaterialStateProperty.all(const BorderSide(color: Colors.white, width: 2)) : null,
      ),
                  ),
                  ...
                  onPressed: () {
                   setState(() {
                    ...
                    if(pencet>=1 && pencet<=3){
                     NotificationService().showNotification(title: 'Cibu App', body: 'Perhatian: Jika aplikasi ter-refresh maka histori transaksi akan hilang karena bersifat sementara!');
                    }
                        });
                  },
...
if ((showhome==true || showhome==false) && !showhistory && !showtransfer)
 Column(
                  children: [
                    Container(
  ...
  if (result != null) {
    final int inputNumber = result['inputNumber'];
    final String formatmoney = result['formatmoney'];
    setState(() {
      _totalNumber += inputNumber;
      saveValue(_totalNumber);
      storeData();
      transactionDataList.add({
        'time': DateFormat('kk:mm:ss').format(DateTime.now()), // 24-hour format
        'amount': formatmoney,
        'phone': "anda sendiri",
        'topup' : inputNumber,
        'transfer': "Top Up",
...
child: Column(
  children: [
                  ...
                  children: [
                    Align(
                      ...
                      children:[
                        ...
                        Column(
              children: [
                GradientText('Masukkan jumlah uang :',
                  style: const TextStyle(
                    fontSize: 25,
                    height: 2
                  ),
    ...
    /*  onChanged: (value) {
            ...
            decoration: InputDecoration(
            icon: const Icon(Icons.format_list_numbered, color: Colors.white, size: 35),
            // prefixIcon: Icon(Icons.phone_iphone, color: Colors.white), 
             ...
             // suffix: const Icon(Icons.keyboard, color: Colors.black, size: 25),
              ...
              hintStyle: const TextStyle(color: Colors.black45, fontSize: 15, fontWeight: FontWeight.w500, height: 2),
            ...
            validator: (value){
                     if(value == null || value.isEmpty)
                      ...
                      } else if(value.startsWith("0") && !(value.contains(RegExp(r'^0*[1-9]')))){
                  ...
                  onPressed: () async {
                     if (validatorKey.currentState != null) {
                      if(validatorKey.currentState!.validate())
                      {
                         ...
                         final int validasiphone = int.tryParse(phoneController.text) ?? 0;
                         bool isExistingUser = await UserDatabase.checkExistingUserByPhone("+${selectedCountry.phoneCode}$validasiphone");
 /* if (isExistingUser) {
  print("ada");
  ...
  print("$validasiphone");
                ...
                if((decreaseNumber>_totalNumber) && isExistingUser && (ap.userModel.phoneNumber != '+${selectedCountry.phoneCode}$validasiphone')){
                  setState(() {
                  _totalNumber -= 0;
                  saveValue(_totalNumber);
                   storeData();
                  });
                        NotificationService().showNotification(title: 'Cibu App', body: 'Saldo tidak mencukupi untuk transfer karena tersisa Rp.$formattedMoney!'); //$_totalNumber
                      } else if(!isExistingUser && (ap.userModel.phoneNumber != '+${selectedCountry.phoneCode}$validasiphone')){
                        NotificationService().showNotification(title: 'Cibu App', body: 'Nomor +${selectedCountry.phoneCode}$validasiphone tidak terdaftar menggunakan App Cibu.');
                      } else if ((decreaseNumber<=_totalNumber) && isExistingUser && (ap.userModel.phoneNumber != '+${selectedCountry.phoneCode}$validasiphone')){

                   final QuerySnapshot<Map<String, dynamic>> result = await FirebaseFirestore
    .instance
    .collection('users')
    .where('phoneNumber', isEqualTo: "+${selectedCountry.phoneCode}$validasiphone")
    .get();

  final UserModel receiver = UserModel.fromMap(result.docs.first.data() as Map<String, dynamic>);
  final int receiverCurrentBalance = int.tryParse(receiver.money) ?? 0;
  receiver.money = (receiverCurrentBalance + decreaseNumber).toString();
                        

                        setState(() {
                  _totalNumber -= decreaseNumber.toInt();
                  saveValue(_totalNumber);
                   storeData();
                  });
    ...
    final formatters = NumberFormat("#,###", "en_US");
    final formattedMoneys = formatters.format(int.parse(decreaseNumber.toString())).replaceAll(',', '.');
    final formatternum = NumberFormat("#,###", "en_US");
    final formattednums = formatternum.format(int.parse(_totalNumber.toString())).replaceAll(',', '.');

                        NotificationService().showNotification(title: 'Cibu App', body: 'Uang berhasil ditransfer ke +${selectedCountry.phoneCode}$validasiphone sebesar Rp.$formattedMoneys!. Sisa saldo anda: Rp.$formattednums.');

                        _onButtonTap();
                        tapCount++;

                      } else if ('+${selectedCountry.phoneCode}$validasiphone' == ap.userModel.phoneNumber){
                        NotificationService().showNotification(title: 'Cibu App', body: 'Tidak boleh memasukkan nomor anda sendiri "+${selectedCountry.phoneCode}$validasiphone".');
                      }

                         
                      }
                      else if((value == null || value.isEmpty) || ((transfervalue.text==null) || (transfervalue.text==null))){
                       setState(() {
                        checks = false;
                       });
                      } else {
...
class UserDatabase {
  static Future<bool> checkExistingUserByPhone(String phone) async {
    QuerySnapshot<Map<String, dynamic>> result = await FirebaseFirestore.instance.collection('users').where('phoneNumber', isEqualTo: phone).get();
