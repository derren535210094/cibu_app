    ...
    /*  onChanged: (value) {
            ...
            validator: (value){
             ...
             ElevatedButton(
               ...
               final inputNumber = int.tryParse(topupvalue.text) ?? 0;
               // Navigator.pop(context, inputNumber);

                final format = NumberFormat("#,###", "en_US");
    final formatmoney = format.format(int.parse(inputNumber.toString())).replaceAll(',', '.');
    Navigator.pop(context, {'inputNumber': inputNumber, 'formatmoney': formatmoney});

                NotificationService().showNotification(title: 'Cibu App', body: 'Uang berhasil masuk ke saldo anda sebesar Rp.$formatmoney!');
                      }
                      else if((value == null || value.isEmpty) || ((topupvalue.text==null) || (topupvalue.text==null))){
                       setState(() {
                        checks = false;
                       });
                      } else {
                ...
                final int inputUSD = int.tryParse(topupvalue.text) ?? 0;
                final int inputNumber = inputUSD*15000;
               // Navigator.pop(context, inputNumber);

                final formatss = NumberFormat("#,###", "en_US");
    final formatmoneys = formatss.format(int.parse(inputNumber.toString())).replaceAll(',', '.');
    Navigator.pop(context, {'inputNumber': inputNumber, 'formatmoney': formatmoneys});

                NotificationService().showNotification(title: 'Cibu App', body: 'Uang berhasil masuk ke saldo anda sebesar Rp.$formatmoneys!');
